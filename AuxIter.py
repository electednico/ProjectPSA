...
class System:
    def __init__(self, p_dict, q_dict, voltage_dict, delta_dict, slack_node):
        """
    ...
    def fill_nodes_dict(self, p_dict, q_dict, voltage_dict, delta_dict):
        ...
        for node_num in p_dict:
            self.nodes_dict[int(node_num)] = Node(p_dict[node_num], q_dict[node_num], voltage_dict[node_num], delta_dict[node_num])
            ...
            if i == self.slack_node:
                ...
                for j in nodes:
                    try:
                        ...
                        nodes[i].p_calc += abs(y_bus[i-1, j-1]) * nodes[i].voltage * nodes[j].voltage * np.cos(nodes[i].delta - nodes[j].delta - ma.phase(y_bus[i-1, j-1]))
                        nodes[i].q_calc += abs(y_bus[i-1, j-1]) * nodes[i].voltage * nodes[j].voltage * np.sin(nodes[i].delta - nodes[j].delta - ma.phase(y_bus[i-1, j-1]))
    ...
    def create_jacobian(self, y_bus):
        ...
        for i in range(self.n):
            ...
            for j in range(self.n):
                if i == j:
                    self.jacobian[i, j] = -nodes[i+i_offset].q_calc - y_bus[i+1, i+1].imag * nodes[i+i_offset].voltage * nodes[i+i_offset].voltage
                else:
                    self.jacobian[i, j] = abs(nodes[i+i_offset].voltage) * abs(nodes[j+j_offset].voltage) * (
                                y_bus[i+1, j+1].real * np.sin(nodes[i+i_offset].delta - nodes[j+j_offset].delta) - y_bus[i+1, j+1].imag * np.cos(
                            nodes[i+i_offset].delta - nodes[j+j_offset].delta))
            # J2 of Jacobian
            for j in range(self.n_pq):
                if i == j:
                    self.jacobian[i, j+self.n] = nodes[i+i_offset].p_calc / abs(nodes[i+i_offset].voltage) + y_bus[i+1, i+1].real * abs(nodes[i+i_offset].voltage)
                else:
                    self.jacobian[i, j+self.n] = abs(nodes[i+i_offset].voltage) * (y_bus[i+1, j+1].real * np.cos(nodes[i+i_offset].delta - nodes[j+j_offset].delta) + y_bus[i+1, j+1].imag * np.sin(
                        nodes[i+i_offset].delta - nodes[j+j_offset].delta))
        ...
        for i in range(self.n_pq):
            ...
            for j in range(self.n):
                if i == j:
                    self.jacobian[i+self.n, j] = nodes[i+i_offset].p_calc - y_bus[i+1, i+1].real * nodes[i+i_offset].voltage * nodes[i+i_offset].voltage
                else:
                    self.jacobian[i+self.n, j] = -abs(nodes[i+i_offset].voltage) * abs(nodes[j+j_offset].voltage) * (y_bus[i+1, j+1].real * np.cos(nodes[i+i_offset].delta - nodes[j+j_offset].delta) + y_bus[i+1, j+1].imag * np.sin(nodes[i+i_offset].delta - nodes[j+j_offset].delta))

            for j in range(self.n_pq):
                # J4 of Jacobian
                if i == j:
                    self.jacobian[i+self.n, j + self.n] = nodes[i+i_offset].q_calc / abs(nodes[i+i_offset].voltage) - y_bus[i+1, i+1].imag * abs(nodes[i+i_offset].voltage)
                else:
                    self.jacobian[i+self.n, j + self.n] = abs(nodes[i+i_offset].voltage) * (y_bus[i+1, j+1].real * np.sin(nodes[i+i_offset].delta - nodes[j+j_offset].delta) - y_bus[i+1, j+1].imag * np.cos(nodes[i+i_offset].delta - nodes[j+j_offset].delta))
...
class Node:
    ...
    def print_data(self, node_num, slack_node):
        ...
        if node_num == slack_node:
            ...
            s = ""
        print("Node {}: P_spec = {}, Q_spec = {}, voltage = {}, delta = {} deg, P_calc = {}, Q_calc = {}, deltaP = {}, deltaQ = {}".format(node_num, self.p_spec, self.q_spec, round(self.voltage,4), round(self.delta,4), round(self.p_calc,4), round(self.q_calc,4), round(self.delta_p,6), round(self.delta_q,6)) + s)
